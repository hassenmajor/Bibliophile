	...
	<body>
		<header>
			...
			<div>
				...
				<p>
					Vous pouvez ouvrir 📖, modifier 📝, 
					<a href="<%= application.getContextPath()+"/livre.jsp?titre=?" %>">ajouter un nouveu livre</a> 
					📕 ou en supprimer à votre guise.
				</p>
			</div>
		</header>
		<ul>
			<%
				// LivreDao dao = new LivreDao();
				for(Livre livre: dao.getAll()) {
					%>
					<li>
						<a href="<%= application.getContextPath()+"/livre.jsp?titre="+livre.getTitre() %>">
							<img alt="<%= livre.getTitre() %>" src="<%= livre.getImage() %>">
						</a>
						<div>
		...
		<footer>
			<p>
				<a href="https://github.com/hassenmajor" target="_BLANK">Hassen Major</a> 👨‍💻 
				Pour rajouter les ouvrages par defaut, 
				<a href="<%= application.getContextPath() %>/index.jsp?reset=true">cliquez-ici</a>.
			</p>
			<br><br>
		</footer>
	</body>
